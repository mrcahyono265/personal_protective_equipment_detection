...
class VideoHandler:
    ...
    def render(self, frame, battery, flying, mode, rec, trim_lr, speed_pct=100, grid=False, height=0, flight_time=0, lr=0.0, fb=0.0, ud=0.0, yaw=0.0):
        ...
        cv2.putText(frame, f'SPD {speed_pct}%', (320, 42),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        alt_text = f'ALT {height / 100:.1f}m' if height is not None else 'ALT --'
        cv2.putText(frame, alt_text, (440, 42),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        tm_text = f'TM {flight_time // 60:02d}:{flight_time % 60:02d}' if flight_time is not None else 'TM --'
        cv2.putText(frame, tm_text, (560, 42),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
